...
/*
 * SHA1 definitions and prototypes - will be removed in the future
 */
typedef struct {
...
static void SHA1_Update(SHA1_CTX *, const unsigned char *, unsigned int);
static void SHA1_Final(unsigned char[SHA1_SIGNATURE_SIZE], SHA1_CTX *);
	     ...
	     const unsigned int len, unsigned char *out)
{
    ...
    sgin[0].page = virt_to_page(in);
    sgin[0].offset = offset_in_page(in);
    sgin[0].length = len;

    sgout[0].page = virt_to_page(out);
    sgout[0].offset = offset_in_page(out);
    sgout[0].length = len;
...
/*     sg[0].page = virt_to_page(MasterKey); */
/*     sg[0].offset = offset_in_page(MasterKey); */
/*     sg[0].length = SessionKeyLength; */
/*     crypto_digest_update(state->sha1_tfm, sg, 1); */

/*     sg[0].page = virt_to_page(SHAPad1); */
/*     sg[0].offset = offset_in_page(SHAPad1); */
/*     sg[0].length = sizeof(SHAPad1); */
/*     crypto_digest_update(state->sha1_tfm, sg, 1); */

/*     sg[0].page = virt_to_page(SessionKey); */
/*     sg[0].offset = offset_in_page(SessionKey); */
/*     sg[0].length = SessionKeyLength; */
/*     crypto_digest_update(state->sha1_tfm, sg, 1); */

/*     sg[0].page = virt_to_page(SHAPad2); */
/*     sg[0].offset = offset_in_page(SHAPad2); */
/*     sg[0].length = sizeof(SHAPad2); */
/*     crypto_digest_update(state->sha1_tfm, sg, 1); */
...
/*
 * SHA1 functions implementation - will be removed in the future
 */
