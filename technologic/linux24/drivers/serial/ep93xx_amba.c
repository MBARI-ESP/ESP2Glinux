...
#define UART_GET_FR(p)			(ts7200_uart_get_fr(p))
...
static int ts7200_auto485 (struct uart_port *port, unsigned int cmd, unsigned long * arg)
{
	int baud, cflag, mode;
	int datalength ; //datalength + parenb
	struct sbc_info *sbc;
	...
	if (!sbc->auto485) {
		remove_sbcinfo(sbc);
		printk("ep93xx_amba.c: sorry, this board does not support auto485\n");
		printk("check /proc/SBC/info for features supported\n");
		return -1;
	}
	...
	switch (cmd) {
		case TIOC_SBCC485:
			if ((mode & AUTO485FD) || (mode & AUTO485HD)) {
				printk("unsetting automode\n");
				outw(0, TS7XXX_RS485CTRL);
				outw(0, TS7XXX_RS485DATA9);
			}
			break;
		case TIOC_SBCS485:
			if (mode & AUTO485FD) {
				printk ("setting FULL duplex\n");
				outw(0x01, TS7XXX_RS485CTRL);
				if (datalength > 8)
					outw(0x01, TS7XXX_RS485DATA9); 
			} else if (mode & AUTO485HD) {
				printk("setting HALF DUPLEX\n");
				switch (baud) {
					case 9600:
						outw(0x04, TS7XXX_RS485CTRL);
						break;
					case 19200:
						outw(0x05, TS7XXX_RS485CTRL);
						break;
					case 57600:
						outw(0x06, TS7XXX_RS485CTRL);
						break;
					case 115200:
						outw(0x07, TS7XXX_RS485CTRL);
						break;
					default:
					printk("auto485.c: %d Baud Rate is not supported for auto RS485 mode\n", baud);
					return -1;
				}
				if (datalength > 8)
					outw(0x01, TS7XXX_RS485DATA9); 
...
static int ep93xxamba_ioctls (struct uart_port *port, unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
		case TIOC_SBCC485:
		case TIOC_SBCS485:
			return ts7200_auto485 (port,cmd, (unsigned long *)arg);
			break;
		default:
			return -ENOIOCTLCMD;
	}
...
static struct uart_amba_port amba_ports[UART_NR] = {
	...
	{
		.port	= {
			.membase	= (void *)IO_ADDRESS(UART3_BASE),
			.mapbase	= UART3_BASE,
			.iotype		= SERIAL_IO_MEM,
			.irq		= IRQ_UART3,
			.uartclk	= 14745600,
