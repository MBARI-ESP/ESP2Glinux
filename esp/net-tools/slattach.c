 ...
 * 	          [-c cmd] [-i cmd] [-s speed] [-p protocol] [-P pidfn] | -
 *
...
#endif
#undef _UID_UUCP  //brent@mbari.org -- no UUCP user
...
usage(void)
{
  char *usage_msg = "Usage: slattach [-ehlLmnqv] "
	  ...
	  "[-o outfill] "
#endif
	  "[-c cmd] [-i cmd] [-s speed] [-p protocol] [-P pidfn] tty | -\n"
	  "       slattach -V | --version\n";
...
main(int argc, char *argv[])
{
  ...
  opterr = 0;
  while ((s = getopt_long(argc, argv, "c:ehlLmnp:qs:vdVk:o:i:P:", longopts, NULL)) != EOF) switch(s) {
	case 'c':
		extcmd = optarg;
		break;
