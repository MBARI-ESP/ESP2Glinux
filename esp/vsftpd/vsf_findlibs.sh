...
locate_library() { [ ! "$PREFIX$1*" = "`echo $PREFIX$1*`" ]; }
...
if [ -r $PREFIX/etc/mandrake-release ]; then
  if [ -r $PREFIX/usr/include/security/pam_appl.h ]; then
    # Yes, Mandrake's PAM installation is broken
    echo "/lib/libpam.so.0";
  else
    echo "-lcrypt";
  fi
  if [ -r $PREFIX/usr/include/sys/capability.h ]; then
    echo "-lcap";
  fi
  exit
elif [ -r $PREFIX/etc/redhat-release ]; then
  if [ -r $PREFIX/usr/include/security/pam_appl.h ]; then
    echo "-lpam";
    grep '6\.' $PREFIX/etc/redhat-release >/dev/null && echo "-ldl"
    grep '5\.' $PREFIX/etc/redhat-release >/dev/null && echo "-ldl"
  else
    echo "-lcrypt";
  fi
